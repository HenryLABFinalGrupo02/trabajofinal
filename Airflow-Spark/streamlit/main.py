...
with open(r'config.yaml','rb') as file: #Open the config.yaml file and then use it in user authentication
...
###### MENU ######
 ################
   ...
   if premium:
      if selected2 == "Home":
         st.title(('Welcome to Vocado Admin Center, '+str(username)))
         tf.load_buss_list(username)
         tf.metricas()
