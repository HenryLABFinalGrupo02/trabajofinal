...
class Authenticate:
            ...
            try:
                if self._check_pw():
                    if inplace:
                        ...
                        st.session_state['premium'] = self.credentials['usernames'][self.username]['premium']
    ...
    def login(self, form_name: str, location: str='main') -> tuple:
        ...
        if not st.session_state['authentication_status']:
            ...
            if not st.session_state['authentication_status']:
                ...
                #premium = self.credentials['usernames'][self.username]
                st.session_state['username'] = self.username
                #premium = self.credentials['usernames'][st.session_state['username']]['premium']
                ...
                #    premium = self.credentials['usernames'][self.username]['premium']
                #else:
                #    premium = None
    

        return st.session_state['name'], st.session_state['authentication_status'], st.session_state['username'],st.session_state['premium']
    ...
    def _register_credentials(self, username: str, name: str, password: str, email: str, preauthorization: bool,premium:bool):
        """
        ...
        self.credentials['usernames'][username] = {'name': name, 
            'password': Hasher([password]).generate()[0], 'email': email,'premium':premium}
        if preauthorization:
    ...
    def register_user(self, form_name: str, location: str='main', preauthorization=True) -> bool:
        ...
        new_premium = register_user_form.text_input('Credit card number') 
        if new_premium == '':
            new_premium =False
        elif new_premium != None:
            new_premium= True
        else:
            new_premium=False
                    ...
                    if new_password == new_password_repeat:
                        if preauthorization:
                            if new_email in self.preauthorized['emails']:
                                self._register_credentials(new_username, new_name, new_password, new_email, preauthorization,new_premium)
                                return True
                            else:
